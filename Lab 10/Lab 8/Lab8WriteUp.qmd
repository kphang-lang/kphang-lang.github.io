...
```{r}
#| label: fig-beta
#| fig-cap: "The Beta Distribution with $α = 7$, $β = 3$"
#| fig-width: 5
#| fig-height: 3
#| scale: "80%"
#| fig-align: "center"
...
```{r}
#| label: fig-bbinomial
#| fig-cap: "The Beta Binomial Distribution with $α = 7$, $β = 3$"
#| fig-width: 5
#| fig-height: 3
#| scale: "80%"
#| fig-align: "center"
...
PMF.plot = ggplot(ggdat_long) + 
  geom_linerange(aes(x = x, ymin = 0, ymax = PMF, fill = theta), 
...
As $θ$ changes, certain outcomes become more or less likely in a best 2 out of 3 match. A $θ$ under $1$ (0.5), favors more extreme options, which is why you see more observations of $3$ wins and a few of $1$ wins. When $θ = 1$, the outcomes are more evenly distributed because each game is being interpreted individually and has the same probability of winning. When $θ = 1.5$, it favors more moderate outcomes, which is why these results are more concentrated in the middle (1-2).
...
The probability of Player A winning the match is the same as the Binomial and Negative Binomial results when $θ = 1$ (0.784), but the probability is higher when $θ = 0.5$ (0.859, is observable also in the graphs). The probability is lower when $θ = 1.5$ (0.747, which is also observable in the graphs).
...
#| fig-cap: "PMF of Altham's Multiplicative Beta-Binomial Distribution with $α = 7$, $β = 3$"
#| fig-width: 5
#| fig-height: 3
#| scale: "80%"
#| fig-align: "center"
