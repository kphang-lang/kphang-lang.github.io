...
```{r}
#| eval: false
vignette(package = "esquisse", lib.loc = NULL, all = TRUE)
```

No demo is available for the esquisse package, but there are two vignettes: get-started and shint-usage.
...
For part i it is sufficient to use a repeating sequence from 1 to 10 to show how each ID received 3 treatments. Part ii, then, assigns the 3 treatments to the corresponding participants using each, but this function needs to do each 10 times to match the 30 IDs.
