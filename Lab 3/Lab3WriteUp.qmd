...
```{r}
(M=matrix(data=c(1, 0, 0, 1), nrow=2, ncol=2, byrow=T)) # storing M to use for part b
```
...
**Solution**
```{r}
(A = matrix(data=c(1, -7, 5, 9), nrow=2, ncol=2, byrow=T)) 
```
...
**Solution**
```{r}
(basis.vector1 = T[ , 1]) # Select columns of T
(basis.vector2 = T[ , 2])
```
...
(p1 = c(1, 5))
(p2 = c(-6, 14))
(p3 = c(-7, 9)) # Store points as vectors
```
...
ggplot() +
  ...
  # xlim(-15, 15) +
  # ylim(-5, 25) +
  labs(x = "x", y = "y") #Plot unit square based on new points in part d
```
...
**Solution**
```{r}
(A = matrix(data = c(1, 2, 2, 4), nrow=2, ncol=2, byrow=T)) 
```
...
```{r}
(basis.vector1 = T[ , 1])
(basis.vector2 = T[ , 2]) # Select columns of T
```
...
(p1 = c(1, 2))
(p2 = c(3, 6))
(p3 = c(2, 4)) # Store points as vectors
```
...
ggplot() +
  ...
  # xlim(-15, 15) +
  # ylim(-5, 25) +
  labs(x = "x", y = "y") # Plot unit square with points from part d
```
...
```{r}
M = matrix(data = c(1, 0.5, 0, 1), nrow=2, ncol=2, byrow=T)
A = matrix(data = c(1, -7, 5, 9), nrow=2, ncol=2, byrow=T)
(T = A%*%M) #Matrix multiplication
```
```{r}
det(T) # Calculate area
```
```{r}
(p0 = c(0, 0))
(p1 = c(1, 5))
(p2 = c(-5.5, 16.5))
(p3 = c(-6.5, 11.5)) # Store points as vectors
```
...
ggplot() +
  ...
  # xlim(-15, 15) +
  # ylim(-5, 25) +
  labs(x = "x", y = "y") # Plot unit square after vertical shearing
```
...
```{r}
M = matrix(data = c(1, 0, 0.5, 1), nrow=2, ncol=2, byrow=T)
(T = A%*%M) # Matrix multiplication 
```
```{r}
det(T) # Calculate area
...
(p1 = c(-2.5, 9.5))
(p2 = c(-9.5, 18.5))
(p3 = c(-7, 9)) # Store the points as vectors
```
...
ggplot() +
  ...
  # xlim(-15, 15) +
  # ylim(-5, 25) +
  labs(x = "x", y = "y") # Plot the unit square with a horizontal shear
```
...
```{r}
M = matrix(data = c(1, 0.5, 0.5, 1), nrow=2, ncol=2, byrow=T)
(T = A%*%M) # Matrix Multiplication
```
```{r}
det(T) # Calculate area
...
(p1 = c(-2.5, 9.5))
(p2 = c(-9, 21))
(p3 = c(-6.5, 11.5)) # Store the points as vectors
```
...
ggplot() +
  ...
  # xlim(-15, 15) +
  # ylim(-5, 25) +
  labs(x = "x", y = "y") # Plot the unit square with both a horizontal and vertical shear
```
...
group = rep(1:11, each =2) # Store the data in vectors
ladder = data.frame(x, y, group) # Create a data frame
```
...
**Solution**
```{r}
A = as.matrix(ladder[, c("x", "y")]) # Make matrix using column one and two of the data frame
M = matrix(data = c(1, 0, 0.5, 1), nrow=2, ncol=2, byrow=T)
(T = A%*%M)
```
...
```{r}
x = T[ , 1]
y = T[ , 2] # Select columns of T
leaning.ladder = data.frame(x, y, group) # Data frame with new values of x and y
```
## Part e
...
ggplot() +
  ...
  xlim(-2,22) +
  ylim(-2,22) +
  scale_color_manual("", values=c("darkred", "grey")) # Plot leaning ladder
```
...
## Citations
Only used @R and related packages for this assignment.