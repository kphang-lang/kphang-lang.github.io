...
Probability statement: $P(X≤1)=P(X=0)+P(X=1)$
...
**Solution**
```{r}
alpha = 7 # define parameters
beta = 3

E_P = alpha / (alpha + beta) # compute mean
Var_P = (alpha * beta) / ((alpha + beta)^2 * (alpha + beta + 1)) # compute variance
```
...
Probability statement: $P(X≥2)=1−P(X≤1)=1−F(1)$
...
1. $P(X≥2∣θ=0.5)=1−P(X≤1∣θ=0.5)$
2. $P(X≥2∣θ=1)=1−F(1;θ=1)$
3. $P(X≥2∣θ=1.5)=1−F(1;θ=1.5)$
...
altham_05 = altham_05 / sum(altham_05)
altham_15 = altham_15 / sum(altham_15)
```
...
altbb_05 = altbb_05 / sum(altbb_05)
altbb_15 = altbb_15 / sum(altbb_15)
```
